import {
  BadgeDollarSign, Boxes, Building2, ChartNoAxesCombined, CircleDollarSign, Columns3, FileUp, LayoutDashboard, LogOut, Menu, PackagePlus, Receipt,
...
export type PageKey='dashboard'|'sales'|'orders'|'returns'|'customers'|'products'|'inventory'|'entries'|'pricing'|'suppliers'|'purchases'|'finance'|'receivables'|'expenses'|'reports'|'import'|'settings';
...
const items:Array<{key:PageKey;label:string;icon:typeof LayoutDashboard;section:string}>=[
  ...
  {key:'import',label:'Importar dados',icon:FileUp,section:'Sistema'},
...
export function AppShell({page,onPage,data,onNewSale,onLogout,children}:{page:PageKey;onPage:(p:PageKey)=>void;data:BootstrapData|null;onNewSale:()=>void;onLogout:()=>void;children:ReactNode}){
  ...
  const results=useMemo(()=>{const term=q.trim().toLowerCase();if(!term||!data)return[];const out:Array<{kind:string;title:string;sub:string;page:PageKey}>=[];data.customers.filter(c=>`${c.name} ${c.phone} ${c.instagram||''}`.toLowerCase().includes(term)).slice(0,3).forEach(c=>out.push({kind:'Cliente',title:c.name,sub:c.phone,page:'customers'}));data.products.filter(p=>`${p.name} ${p.category}`.toLowerCase().includes(term)).slice(0,3).forEach(p=>out.push({kind:'Produto',title:p.name,sub:`${p.variants.reduce((n,v)=>n+v.stock,0)} un. em estoque`,page:'products'}));data.sales.filter(s=>`${s.number} ${s.customerName}`.toLowerCase().includes(term)).slice(0,3).forEach(s=>out.push({kind:'Pedido',title:s.number,sub:s.customerName,page:'sales'}));data.suppliers.filter(s=>`${s.name} ${s.phone||''} ${s.instagram||''} ${s.cnpj||''}`.toLowerCase().includes(term)).slice(0,2).forEach(s=>out.push({kind:'Fornecedor',title:s.name,sub:s.phone||s.instagram||'Fornecedor',page:'suppliers'}));data.purchases.filter(p=>`${p.number} ${p.supplierName}`.toLowerCase().includes(term)).slice(0,2).forEach(p=>out.push({kind:'Compra',title:p.number,sub:p.supplierName,page:'purchases'}));return out},[q,data]);
