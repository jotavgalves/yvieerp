import {
  BadgeDollarSign, Boxes, Building2, ChartNoAxesCombined, Columns3, LayoutDashboard, LogOut, Menu, PackagePlus,
  Search, Settings2, ShoppingBag, ShoppingCart, Shirt, Users, WalletCards, X
} from 'lucide-react';
...
export type PageKey='dashboard'|'sales'|'orders'|'customers'|'products'|'inventory'|'entries'|'pricing'|'suppliers'|'purchases'|'finance'|'reports'|'settings';
...
export function AppShell({page,onPage,data,onNewSale,onLogout,children}:{page:PageKey;onPage:(p:PageKey)=>void;data:BootstrapData|null;onNewSale:()=>void;onLogout:()=>void;children:ReactNode}){
  ...
  const pendingPurchases=data?.purchases.filter(p=>p.status==='Pedido').length||0;
  const results=useMemo(()=>{
    ...
    data.suppliers.filter(s=>`${s.name} ${s.phone||''} ${s.instagram||''} ${s.cnpj||''}`.toLowerCase().includes(term)).slice(0,2).forEach(s=>out.push({kind:'Fornecedor',title:s.name,sub:s.phone||s.instagram||'Fornecedor',page:'suppliers'}));
    data.purchases.filter(p=>`${p.number} ${p.supplierName}`.toLowerCase().includes(term)).slice(0,2).forEach(p=>out.push({kind:'Compra',title:p.number,sub:p.supplierName,page:'purchases'}));
    return out;
  },[q,data]);
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen?'sidebar-open':''}`}>
      ...
      <nav className="nav">{sections.map(section=><div key={section} className="nav-group"><span className="nav-section">{section}</span>{items.filter(i=>i.section===section).map(item=>{const Icon=item.icon;return <button key={item.key} className={`nav-item ${page===item.key?'active':''}`} onClick={()=>{onPage(item.key);setMobileOpen(false)}}><Icon size={18}/><span>{item.label}</span>{item.key==='orders'&&pending>0&&<b>{pending}</b>}{item.key==='purchases'&&pendingPurchases>0&&<b>{pendingPurchases}</b>}</button>})}</div>)}</nav>
