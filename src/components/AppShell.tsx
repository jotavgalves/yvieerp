import {
  BadgeDollarSign, Boxes, ChartNoAxesCombined, Columns3, LayoutDashboard, LogOut, Menu, PackagePlus,
...
export type PageKey='dashboard'|'sales'|'orders'|'customers'|'products'|'inventory'|'entries'|'pricing'|'finance'|'reports'|'settings';
...
export function AppShell({page,onPage,data,onNewSale,onLogout,children}:{page:PageKey;onPage:(p:PageKey)=>void;data:BootstrapData|null;onNewSale:()=>void;onLogout:()=>void;children:ReactNode}){
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen?'sidebar-open':''}`}>
      ...
      <nav className="nav">{sections.map(section=><div key={section} className="nav-group"><span className="nav-section">{section}</span>{items.filter(i=>i.section===section).map(item=>{const Icon=item.icon;return <button key={item.key} className={`nav-item ${page===item.key?'active':''}`} onClick={()=>{onPage(item.key);setMobileOpen(false)}}><Icon size={18}/><span>{item.label}</span>{item.key==='orders'&&pending>0&&<b>{pending}</b>}</button>})}</div>)}</nav>
