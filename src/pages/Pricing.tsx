...
export function Pricing({openSignal=0,initialProductId=''}:{openSignal?:number;initialProductId?:string}){
  ...
  useEffect(()=>{
    ...
    setFreightCost(last?.freightCost||0);
    setOtherCost(last?.otherCost||0);
  ...
  async function saveProductRule(){
    ...
    try{
      const saved=await api<{targetMargin:number;cardFee:number;inheritedVariants:number}>(`/api/pricing/products/${product.id}/settings`,{method:'PUT',body:JSON.stringify({targetMargin:productMargin,cardFee:productCardFee})});
      const loaded=await loadSettings(product.id);
      if(loaded&&variantSetting?.inheritsProduct){setTargetMargin(loaded.defaultTargetMargin);setCardFee(loaded.defaultCardFee)}
      notify(`Regra de ${product.name} salva: ${saved.targetMargin.toFixed(1)}% de margem e ${saved.cardFee.toFixed(2)}% de taxa. ${saved.inheritedVariants} variação(ões) herdam esse padrão.`);
  ...
  async function savePrices(){
    ...
    try{
      if(customVariant){
        await api(`/api/pricing/variants/${variant.id}/settings`,{method:'PUT',body:JSON.stringify({inheritProduct:false,targetMargin,cardFee})});
      }else{
        await api(`/api/pricing/variants/${variant.id}/settings`,{method:'PUT',body:JSON.stringify({inheritProduct:true})});
      }
      const saved=await api<{cashPrice:number;cardPrice:number;pieceCost:number;targetMargin:number;cardFee:number;inheritsProduct:boolean;persisted:boolean}>(`/api/pricing/${variant.id}`,{method:'POST',body:JSON.stringify({freightCost,otherCost})});
      if(!saved.persisted)throw new Error('O banco não confirmou a precificação.');
      await refresh();
      await loadSettings(product.id);
      notify(`Salvo: ${money(saved.cashPrice)} à vista e ${money(saved.cardPrice)} no cartão · margem ${saved.targetMargin.toFixed(1)}% · taxa ${saved.cardFee.toFixed(2)}%.`);
    }finally{setBusy(false)}
  }

  return <>
    <PageHeader title="Precificação" subtitle="Cada produto tem sua própria margem e taxa padrão. Cada variação pode herdar esse padrão ou ter uma regra totalmente própria."/>
    ...
    <div className="pricing-layout">
      <section className="pricing-catalog panel">
        <div className="panel-head"><div><h2>Catálogo</h2><p>As regras são isoladas por produto.</p></div><Calculator size={18}/></div>
      ...
      <section className="pricing-workspace panel">
        {!product||!variant?<EmptyState icon={<Calculator/>} title="Selecione um produto" text="Você poderá definir primeiro a regra padrão do produto e depois personalizar variações específicas."/>:<>
          <div className="pricing-workspace-head"><div><span className="eyebrow">{product.category}</span><h2>{product.name}</h2><p>Custo médio real da variação {money(variant.averageCost)} · à vista {money(variant.cashPrice)} · cartão {money(variant.cardPrice)}</p></div><div className="pricing-current-margin"><span>Margem atual</span><strong>{margin(variant.cashPrice,variant.averageCost).toFixed(1)}%</strong></div></div>

          <div className="pricing-block">
            <div className="pricing-block-title"><span>P</span><div><strong>Regra padrão de {product.name}</strong><small>Essa margem e essa taxa pertencem somente a este produto. Variações sem personalização herdam os dois valores.</small></div></div>
            <div className="form-grid"><Field label="Margem padrão do produto %"><Input type="number" min="0" max="95" step=".1" value={productMargin} onChange={e=>setProductMargin(Math.min(95,Math.max(0,Number(e.target.value))))}/></Field><Field label="Taxa de maquineta padrão %"><Input type="number" min="0" max="40" step=".01" value={productCardFee} onChange={e=>setProductCardFee(Math.min(40,Math.max(0,Number(e.target.value))))}/></Field></div>
            <div className="pricing-apply"><div><strong>Salvar padrão deste produto</strong><span>Não altera a regra dos outros produtos e não mexe nas variações que estejam personalizadas.</span></div><Button loading={settingsBusy} onClick={()=>void saveProductRule()}>Salvar regra do produto</Button></div>
          </div>
          ...
          <div className="pricing-form-grid">
            <div className="pricing-block"><div className="pricing-block-title"><span>1</span><div><strong>Composição de custo</strong><small>O custo da peça vem do estoque. Frete e outros custos continuam próprios desta variação e desta precificação.</small></div></div><div className="form-grid"><Field label="Custo médio real" helper="Para corrigir custo, use Estoque → Ajustar."><Input type="number" min="0" step=".01" value={pieceCost} disabled/></Field><Field label="Frete por peça"><Input type="number" min="0" step=".01" value={freightCost} onChange={e=>setFreightCost(Number(e.target.value))}/></Field><Field label="Outros custos" className="span-2"><Input type="number" min="0" step=".01" value={otherCost} onChange={e=>setOtherCost(Number(e.target.value))}/></Field></div><div className="pricing-total-line"><span>Custo usado para formar o preço</span><strong>{money(totalCost)}</strong></div></div>
            <div className="pricing-block"><div className="pricing-block-title"><span>2</span><div><strong>Regra desta variação</strong><small>Escolha se ela acompanha {product.name} ou se terá margem/taxa exclusivas.</small></div></div>
              <div className="pricing-mode"><button type="button" className={!customVariant?'active':''} onClick={()=>{setCustomVariant(false);setTargetMargin(settings?.defaultTargetMargin??productMargin);setCardFee(settings?.defaultCardFee??productCardFee)}}>Herdar do produto<br/><strong>{(settings?.defaultTargetMargin??productMargin).toFixed(1)}% · {(settings?.defaultCardFee??productCardFee).toFixed(2)}%</strong></button><button type="button" className={customVariant?'active':''} onClick={()=>{setCustomVariant(true);setTargetMargin(variantSetting?.effectiveTargetMargin??settings?.defaultTargetMargin??productMargin);setCardFee(variantSetting?.effectiveCardFee??settings?.defaultCardFee??productCardFee)}}>Personalizar variação<br/><strong>{customVariant?'Regra própria':'Ativar'}</strong></button></div>
              <div className="form-grid"><Field label="Margem desta variação %" helper={customVariant?'Só esta variação será afetada.':'Herdando a margem padrão do produto.'}><Input type="number" min="0" max="95" step=".1" value={effectiveMargin} disabled={!customVariant} onChange={e=>setTargetMargin(Math.min(95,Math.max(0,Number(e.target.value))))}/></Field><Field label="Taxa de maquineta desta variação %" helper={customVariant?'Só esta variação será afetada.':'Herdando a taxa padrão do produto.'}><Input type="number" min="0" max="40" step=".01" value={effectiveCardFee} disabled={!customVariant} onChange={e=>setCardFee(Math.min(40,Math.max(0,Number(e.target.value))))}/></Field></div>
