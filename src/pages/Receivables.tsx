import { CheckCircle2, CircleDollarSign, Clock3, X } from 'lucide-react';
...
export function Receivables(){
  ...
  async function cancel(r:Receivable){if(!confirm(`Cancelar a cobrança ${r.description}?`))return;setBusy(true);try{await api(`/api/receivables/${r.id}/cancel`,{method:'POST'});await refresh();notify('Cobrança cancelada.')}finally{setBusy(false)}}
  return <>
    <PageHeader title="A receber" subtitle="Separe venda registrada de dinheiro efetivamente recebido."/>
