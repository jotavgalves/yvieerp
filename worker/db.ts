...
export async function bootstrap(env:Env){
  const [customersR,productsR,variantsR,salesR,itemsR,entriesR,expensesR,pricingR,suppliersR,purchasesR,purchaseItemsR,receivablesR,movementsR,returnsR,returnItemsR,creditsR,countsR,countItemsR,ownerR]=await Promise.all([
    ...
    env.DB.prepare(`SELECT r.*,s.number AS sale_number,c.name AS customer_name FROM returns r JOIN sales s ON s.id=r.sale_id JOIN customers c ON c.id=s.customer_id WHERE s.deleted_at IS NULL ORDER BY r.created_at DESC`).all(),
    env.DB.prepare(`SELECT ri.*,p.name AS product_name,v.color,v.size FROM return_items ri JOIN returns r ON r.id=ri.return_id JOIN sales s ON s.id=r.sale_id JOIN products p ON p.id=ri.product_id JOIN product_variants v ON v.id=ri.variant_id WHERE s.deleted_at IS NULL ORDER BY ri.rowid ASC`).all(),
