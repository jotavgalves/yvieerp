...
export async function bootstrap(env:Env){
  const [customersR,productsR,variantsR,salesR,itemsR,entriesR,expensesR,pricingR]=await Promise.all([
  ...
  return {customers,products,sales,entries,expenses,pricing,summary:{revenue,grossProfit:revenue-cost,expenses:expenseTotal,netProfit:revenue-cost-expenseTotal,orders:validSales.length,ticketAverage:validSales.length?revenue/validSales.length:0,stockUnits,stockCost}};
}
