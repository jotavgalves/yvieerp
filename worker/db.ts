...
export async function bootstrap(env:Env){
  const [customersR,productsR,variantsR,salesR,itemsR,entriesR,expensesR,pricingR,suppliersR,purchasesR,purchaseItemsR]=await Promise.all([
  ...
  return {
    customers,products,sales,entries,expenses,pricing,suppliers,purchases,
    summary:{
      ...
      stockUnits,stockCost,stockPotentialRevenue,stockPotentialProfit,pendingPurchases
    }
  };
}
