...
export async function createReturn(request:Request,env:Env,saleId:string){
  ...
  for(const raw of exchange){
    ...
    const unitPrice=Math.max(0,number(raw.unitPrice,variant.sale_price));exchangeValue+=quantity*unitPrice;
  ...
  const suggested=Math.max(0,returnedValue-exchangeValue);
  const refund=Math.max(0,number(input.refundAmount,suggested));
